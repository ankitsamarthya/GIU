...
genratePredictedMap<-function(model.type='logistic',T1File,T2File,with.class.name=NA,T1drivers,T2drivers,withNAvalue,outputFile,demand=NA,restrictSpatialMigration=NA,AllowedClassMigration=NA,
				conversionOrder='TP',classAllocationOrder=NA,neighbour=NA,method='random')
{
	...
	#writeRaster(predictedMapSingle,outputFile,format='GTiff', options="INTERLEAVE=BIP",datatype='INT1U')
	return(predictedMapSingle)
}
